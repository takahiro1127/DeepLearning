...
from keras.layers import Input, Dense, Embedding, LSTM, Activation, Flatten, Reshape, dot, Permute, Lambda, CuDNNLSTM, Dropout, Bidirectional
...
lstm = Bidirectional(LSTM(hid_dim, return_sequences=True,
...
h_seq = lstm(y_emb_t)
_, h_t, cell_t = lstm2(y_emb_t, initial_state=[h_tm1, cell_tm1])
...
def beam_search(x, max_len=100, k=4):
...
for i in tqdm(range(x_test.shape[0])):
    pred_y = beam_search(x_test[i:i+1], k=4, max_len=40)
