...
# model.add(Bidirectional(CuDNNLSTM(hid_dim, unit_forget_bias=True,
#                                   bias_initializer="zeros")))
model.add(Bidirectional(CuDNNLSTM(32, return_sequences=True, unit_forget_bias=True,
                                  bias_initializer="zeros"), input_shape=x_train.shape[1:]))
...
model.add(Bidirectional(
    CuDNNLSTM(32, unit_forget_bias=True, bias_initializer="zeros")))
model.add(Dropout(0.1))
# model.add(Bidirectional(CuDNNLSTM(hid_dim, unit_forget_bias=True,
#                                   bias_initializer="zeros")))
# model.add(Dropout(0.1))
...
learning_rate = LearningRateScheduler(
    lambda ep: float(1e-3 / 3 ** (ep * 4 // 50)))
