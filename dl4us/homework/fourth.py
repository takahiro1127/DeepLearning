...
def load_data():
    ...
    tokenizer_en = np.load(
        '/root/userspace/public/lesson4/data/tokenizer_en.npy').item()
    tokenizer_ja = np.load(
        '/root/userspace/public/lesson4/data/tokenizer_ja.npy').item()
...
output = [decode_sequence(x_test[i][np.newaxis, :], bos_eos, 100)[
    1:-1] for i in range(len(x_test))]
